-- Using a first dataset
SELECT * FROM book.lending;

-- 1.	What is the sum of HI-Honolulu’s Loan Amount by “Borrower Referral” for 2022?
SELECT STR_TO_DATE(`Closing Date`, "%m/%d/%Y") ,`(B) REFERRAL OFFICE`, SUM(`LOAN AMOUNT (currency)`) AS "Total Loan Amount"
FROM book.lending
WHERE `(B) REFERRAL OFFICE` LIKE "%Honolulu" AND YEAR(STR_TO_DATE(`Closing Date`, "%m/%d/%Y")) = 2022;

-- 2.	What is the sum of CA-Monrovia’s Loan Amount by “Investor Referral” for 2023 (2.5 points)
SELECT STR_TO_DATE(`Closing Date`, "%m/%d/%Y") ,`(I) REFERRAL OFFICE`, SUM(`LOAN AMOUNT (currency)`) AS "Total Loan Amount"
FROM book.lending
WHERE `(I) REFERRAL OFFICE` LIKE "%CA-Monrovia" AND YEAR(STR_TO_DATE(`Closing Date`, "%m/%d/%Y")) = 2023;

-- 3.	Which office has the highest total Loan Amount (Borrower + Investor Referral) for 2024 


-- 4.	Which office has the lowest total Loan (Borrower + Investor Referral) 
